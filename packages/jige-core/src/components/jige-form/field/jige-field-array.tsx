...
>(props: {
  ...
  children: (props: { items: number[]; name: string }) => JSX.Element;
}) {
  ...
  createWatch(
    ...
    (value) => {
      ...
      const errors = Object.keys(formState.errorFields).filter((key) =>
        key.startsWith(props.name)
      );
      const dirties = Object.keys(formState.dirtyFields).filter((key) =>
        key.startsWith(props.name)
      );
